...
public abstract class CustomDrop extends AbstractDropEvent implements Runnable {
    ...
    public boolean matches(AbstractDropEvent other) {
        ...
        if (other instanceof OccurredEvent) {
            ...
            if (!isTool(drop.getTool())) {
                return false; // TODO: log message is inside isTool check - do this for all?
            }
    ...
    public boolean isTool(Agent tool) {
        ...
        if (!positiveMatch) {
            Log.logInfo("Tool match = " + positiveMatch + " - tool=" + String.valueOf(tool) + " tools=" + tools.toString(), HIGHEST);
        }
...
//		return checkList(sky, weather);  // TODO: (ZAR) doesn't work - but should, no?
    ...
    public boolean inGroup(Agent agent) {
        ...
        if (!(agent instanceof PlayerSubject)) {
            if (agent instanceof ProjectileAgent) {
                ...
                return false; // if permissions is set and agent is not a player, fail
            }
        }
    ...
    public boolean hasPermission(Agent agent) {
        ...
        if (!(agent instanceof PlayerSubject)) {
            ...
            if (player == null) {
                return false; // if permissions is set and agent (or shooter) is not a player, fail
            }
        }
...
//		if(schedule > 0.0) Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(OtherDrops.plugin, this, schedule);
        ...
        if (schedule > 0.0) {
            Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(OtherDrops.plugin, dropRunner, schedule);
        } else {
            dropRunner.run();
        }
        //}
