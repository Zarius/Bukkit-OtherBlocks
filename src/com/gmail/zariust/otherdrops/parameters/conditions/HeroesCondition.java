...
public class HeroesCondition extends Condition {
    ...
    public HeroesCondition(Object toParse, Type type) {
        this.type = type;
        this.parameterValue = (String) toParse; // FIXME: support lists (refer to MessageAction)
        // TODO: even better might be to allow a list of class@level, eg.  [FARMER@1-5, WORKER@6, etc]
    ...
    protected boolean checkInstance(CustomDrop drop, OccurredEvent occurrence) {
        ...
        // Grab the character manager & check values against the saved value/values
    ...
    public List<Condition> parse(ConfigurationNode parseMe) {
        ...
        // return the empty list if no heroes, no use scanning through the parameters at all
        if (!Dependencies.hasHeroes()) {
            return conditions;
        }

        // loop through valid parameters and add a new condition for each one that exists in "parseMe"
        for (String key : matches.keySet()) {
            if (parseMe.get("heroes." + key) != null) {
                conditions.add(new HeroesCondition(parseMe.get(key), matches.get(key)));
            }
        }
        return conditions;
    }
